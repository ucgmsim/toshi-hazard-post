...
def rates_weights(config, hazard_logic_tree):
    ...
    rates_weights = calc_aggregation_arrow(
                    ...
                    logic_tree = hazard_logic_tree, compatibility_key=config.compat_key, hazard_model_id=config.hazard_model_id
                )
    yield rates_weights
...
def test_duckdb_can_sum_scalars(rates_weights):
    ...
    con = duckdb.connect()
    summed = con.execute(
        f"SELECT sources_digest, gmms_digest, sum(weight) FROM rates_weights "
        f"GROUP BY sources_digest, gmms_digest;")
...
def test_duckdb_cannot_sum_arrays(rates_weights):
    ...
    with pytest.raises(Exception):
        summed = con.execute(
            f"SELECT sources_digest, gmms_digest, sum(values), sum(weight) FROM rates_weights "
            f"GROUP BY sources_digest, gmms_digest;")
        print(summed)
...
def test_pandas_with_filter_groupby(rates_weights, hazard_logic_tree):
    ...
    # TODO sum of weights should be 4, but no it's 2
...
def test_original_aggregation(hazard_logic_tree, config):
    ...
    levels = range(44)
    site = sites[0]
    value_store = load_realizations(hazard_logic_tree, imt='PGA', location=site.location,
        vs30=site.vs30, compatibility_key=config.compat_key)
...
def test_apply_fn_with_pandas_concat(rates_weights):
    """ tried to do this in numpy but ???
    # np.apply_along_axis(vpr, 0, val_series, inv_time=1.0)
    """
...
# def test_no_apply_fn_with_pandas(rates_weights):
#     vpr = np.vectorize(calculators.prob_to_rate)
#     rw_df = rates_weights.to_pandas()
#     print(rw_df['rates'].array.shape)
#     def series_vpr(arr, vpr_fn):
#         return vpr_fn(arr, inv_time=1.0)
#     svpr = np.vectorize(series_vpr)
#     val_series = svpr(rw_df['rates'].array, vpr_fn=vpr)
#     print(val_series)
#     assert 0
...
def test_numpy_nested_array_and_vectorized_perf():
    """
    this shows us that pandas it actualy quicker until array sizes get larger than 912 * 10,
    ... then numpy trumps pandas
    """
    _2d_array = []
