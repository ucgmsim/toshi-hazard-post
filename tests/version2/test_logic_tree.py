...
def test_logic_tree_trt(source_logic_tree, gmcm_logic_tree):
    ...
    source_logic_tree.branch_sets = [bs for bs in source_logic_tree.branch_sets if 'Active Shallow Crust' not in bs.tectonic_region_types]
    hazard_logic_tree = HazardLogicTree(source_logic_tree, gmcm_logic_tree)
...
def test_logic_tree_weights(source_logic_tree, gmcm_logic_tree):
    ...
    weight_expected = (
        component_branch.source_branch.weight * 
        reduce(mul, [branch.weight for branch in component_branch.gmcm_branches])
    )
